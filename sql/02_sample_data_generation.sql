...
-- Normal baseline patterns (last 30 days)
INSERT INTO USER_AUTHENTICATION_LOGS 
SELECT 
    'AUTH_' || ROW_NUMBER() OVER (ORDER BY emp.EMPLOYEE_ID, seq.value) as LOG_ID,
    DATEADD(day, -seq.value, CURRENT_TIMESTAMP()) - 
...
CROSS JOIN (SELECT * FROM TABLE(GENERATOR(ROWCOUNT => 30))) seq
...
SELECT 
    ...
    TRUE as SUCCESS
FROM EMPLOYEE_DATA emp
CROSS JOIN (SELECT * FROM TABLE(GENERATOR(ROWCOUNT => 100))) seq
WHERE emp.STATUS = 'active';
