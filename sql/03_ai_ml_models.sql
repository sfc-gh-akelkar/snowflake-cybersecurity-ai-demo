...
SELECT 
    ...
    COUNT(*) as frequency,
    AVG(TIMESTAMPDIFF(MINUTE, LAG(TIMESTAMP) OVER (PARTITION BY USERNAME ORDER BY TIMESTAMP), TIMESTAMP)) as avg_session_gap_minutes
...
WITH user_stats AS (
    SELECT 
        USERNAME,
        COLLECT_LIST(EXTRACT(HOUR FROM TIMESTAMP)) as typical_hours,
        COLLECT_SET(LOCATION:country::STRING) as typical_countries,
        AVG(DATEDIFF(minute, LAG(TIMESTAMP) OVER (PARTITION BY USERNAME ORDER BY TIMESTAMP), TIMESTAMP)) as avg_gap_minutes,
        STDDEV(DATEDIFF(minute, LAG(TIMESTAMP) OVER (PARTITION BY USERNAME ORDER BY TIMESTAMP), TIMESTAMP)) as stddev_gap_minutes
    FROM USER_AUTHENTICATION_LOGS
    WHERE TIMESTAMP >= DATEADD(day, -30, CURRENT_TIMESTAMP())
        AND SUCCESS = TRUE
    GROUP BY USERNAME
